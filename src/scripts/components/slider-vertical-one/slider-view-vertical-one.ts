...
export default class SliderViewVerticalOne extends AbstractView {
  ...
  get template() {
    return `
      <div data-testid="slider-vertical-one" class="slider__inner slider__inner--height">
  ...
  bind() {
    this.element.querySelectorAll('.slider__toggle-vertical').forEach((elem) => elem.addEventListener('mousedown', this.onToggleMouseDown))
  }
