  ...
  get template() {
    return `
      ...
      <div class="slider__input">
        ...
        <input data-name="from" class="slider__from slider__number" type="number" value=${this.sliderModel.fromValue === NULL_VALUE ? this.sliderModel.minValue : this.sliderModel.fromValue} id="from" readonly>
      </div>
      <div class="slider__input">
        <label class="slider__label" for="to">До</label>
        <input data-name="to" class="slider__to slider__number" type="number" value=${this.sliderModel.toValue === NULL_VALUE ? this.sliderModel.maxValue : this.sliderModel.toValue} id="to" readonly>
      </div>
      <div class="slider__input">
      ...
      <div class="slider__input">
        <input data-name="view" class="slider__view" type="radio" name="view${this.sliderModel.mainName}" value="horizontal" id="horizontal${this.sliderModel.mainName}" ${this.sliderModel.viewValue === View.HORIZONTAL ? 'checked' : ''}>
        <label for="horizontal${this.sliderModel.mainName}">Горизонтальный</label>
        <input data-name="view" class="slider__view" type="radio" name="view${this.sliderModel.mainName}" value="vertical" id="vertical${this.sliderModel.mainName}" ${this.sliderModel.viewValue === View.VERTICAL ? 'checked' : ''}>
        <label for="vertical${this.sliderModel.mainName}">Вертикальный</label>
      </div>
      <div class="slider__input">
        <input data-name="range" class="slider__range" type="radio" name="range${this.sliderModel.mainName}" value="one" id="one${this.sliderModel.mainName}" ${this.sliderModel.rangeValue === Range.ONE ? 'checked' : ''}>
        <label for="one${this.sliderModel.mainName}">Одиночное значение</label>
        <input data-name="range" class="slider__range" type="radio" name="range${this.sliderModel.mainName}" value="range" id="range${this.sliderModel.mainName}" ${this.sliderModel.rangeValue === Range.RANGE ? 'checked' : ''}>
